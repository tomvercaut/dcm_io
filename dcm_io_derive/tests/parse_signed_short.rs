...
mod tests {
    ...
    pub struct RequiredField {
        #[dicom(tag = "0010,0020", vr = "SS")]
        pub field: i16,
    }

    #[derive(Dicom, Default, Clone)]
    pub struct RequiredFields {
        #[dicom(tag = "(0012,0063)", vr = "SS")]
        pub field: Vec<i16>,
    }
    ...
    pub struct OptionalField {
        #[dicom(tag = "0010,0020", vr = "SS")]
        pub field: Option<i16>,
    }

    #[derive(Dicom, Default, Clone)]
    pub struct OptionalFields {
        #[dicom(tag = "(0012,0063)", vr = "SS")]
        pub field: Option<Vec<i16>>,
    }
    ...
    fn read_required_dicom_tags_field() {
        ...
        let ime = DataElement::new(
            Tag(0x0012, 0x0063),
            VR::SS,
            dicom_core::PrimitiveValue::I16(vec![10, 20].into()),
        );
        let _ = obj.put_element(ime);
        let required_field = RequiredFieldsReader::read_dicom(&obj).unwrap();
        assert_eq!(required_field.field.len(), 2);
        assert_eq!(required_field.field[0], 10);
        assert_eq!(required_field.field[1], 20);
    }
    ...
    fn read_optional_dicom_tag_field() {
        ...
        assert_eq!(optionalfield.field.as_ref().unwrap(), &42);
    }
    ...
    fn read_optional_dicom_tags_field() {
        ...
        assert_eq!(optional_field.field.as_ref().unwrap()[0], 10);
        assert_eq!(optional_field.field.as_ref().unwrap()[1], 20);
    }
