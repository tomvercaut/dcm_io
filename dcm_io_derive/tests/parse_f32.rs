...
mod tests {
    ...
    pub struct RequiredField {
        #[dicom(tag = "0010,0020", vr = "FL")]
        pub field: f32,
    }

    #[derive(Dicom, Default, Clone)]
    pub struct RequiredFields {
        #[dicom(tag = "(0012,0063)", vr = "FL")]
        pub field: Vec<f32>,
    }
    ...
    fn read_required_dicom_tag_field() {
        ...
        let ime = DataElement::new(
            Tag(0x0010, 0x0020),
            VR::FL,
            dicom_core::PrimitiveValue::F32(vec![3.14].into()),
        );
    ...
    fn read_required_dicom_tags_field() {
        ...
        let ime = DataElement::new(
            Tag(0x0012, 0x0063),
            VR::FL,
            dicom_core::PrimitiveValue::F32(vec![1.5, 2.7].into()),
        );
        let _ = obj.put_element(ime);
        let required_field = RequiredFieldsReader::read_dicom(&obj).unwrap();
        assert_eq!(required_field.field.len(), 2);
        assert_eq!(required_field.field[0], 1.5);
        assert_eq!(required_field.field[1], 2.7);
    }
    ...
    fn read_optional_dicom_tag_field() {
        ...
        assert_eq!(optionalfield.field.as_ref().unwrap(), &3.14);
    }
    ...
    fn read_optional_dicom_tags_field() {
        ...
        assert_eq!(optional_field.field.as_ref().unwrap()[0], 1.5);
        assert_eq!(optional_field.field.as_ref().unwrap()[1], 2.7);
    }
