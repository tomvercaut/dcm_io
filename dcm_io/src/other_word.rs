...
pub fn read_other_word(obj: &InMemDicomObject, tag: Tag) -> crate::Result<u16> {
    let value = obj
        .element(tag)
        .map_err(|e| {
            eprintln!("{:#?}", e);
...
pub fn read_other_words(obj: &InMemDicomObject, tag: Tag) -> crate::Result<Vec<u16>> {
    let value = obj
        .element(tag)
...
pub fn read_other_word_opt(obj: &InMemDicomObject, tag: Tag) -> crate::Result<Option<u16>> {
...
pub fn read_other_words_opt(obj: &InMemDicomObject, tag: Tag) -> crate::Result<Option<Vec<u16>>> {
    match obj.element_opt(tag)? {
        Some(elem) => {
