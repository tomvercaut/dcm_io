...
pub fn read_other_very_long(obj: &InMemDicomObject, tag: Tag) -> crate::Result<u64> {
    let value = obj
        .element(tag)
        .map_err(|e| {
            eprintln!("{:#?}", e);
...
pub fn read_other_very_longs(obj: &InMemDicomObject, tag: Tag) -> crate::Result<Vec<u64>> {
    let value = obj
        .element(tag)
...
pub fn read_other_very_long_opt(obj: &InMemDicomObject, tag: Tag) -> crate::Result<Option<u64>> {
...
pub fn read_other_very_longs_opt(obj: &InMemDicomObject, tag: Tag) -> crate::Result<Option<Vec<u64>>> {
    match obj.element_opt(tag)? {
        Some(elem) => {
